        ...
        <div class="overlay topz flex items-center justify-center w-screen" id="verify-modal">
            ...
            <div class="modal flex flex-col space-y-8 rounded md:w-3/5 w-4/5 bg-gray-200 p-8">
                <div class="modal-header flex justify-between">
                    <h1 class="text-lg leading-6 font-medium text-gray-900">{{__('Verify Account')}}</h1>
                    <!-- <a href="#" class="close text-xl">&times;</a> -->
                </div>
                <div class="modal-body">
                    <p class="text-sm mb-2 text-gray-500">{{__('Code Verification will be sent to your email click send verification.')}}</p>
                    @if(request()->is('*' . 'sendcode'))
                    <a class="text-base font-medium text-indigo-800">
                        {{__('Verification Code was sended.')}}
                    </a>
                    @else
                    ...
                    <form class="mt-8 space-y-6" action="{{route('verify.post')}}" method="POST">
                        ...
                        <div class="rounded-md shadow-sm -space-y-px">
                            <div class="flex flex-col space-y-2 mb-4">
                                ...
                                <label for="ver-code" class="">{{__('Enter Verification Code')}}</label>
