                    ...
                    <ul class="flex flex-col">
                        <li class="bg-white my-2 opinion-item shadow-lg" x-data="accordion(1)">
                            ...
                            <div class="flex justify-between">
                                ...
                                <div>
                                    <p>
                                        By<a href="{{ route('author.opinions',$item->user_id)}}"
                                             class="text-indigo-600 text-sm hover:underline hover:text-indigo-800 ml-2">{{ $item->author->f_name}} {{ $item->author->l_name }} </a>
                                    </p>
                                </div>
                                <form data-route="{{ route('opinions.destroy',$item->id)}}" method="post"
                                      class="delete-form mx-2 my-2 text-indigo-600 hover:text-indigo-900">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                            class="inline-flex text-sm w-16 justify-center py-1 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                        Delete
                                    </button>
                                </form>
                            </div>
