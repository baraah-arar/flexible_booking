                        ...
                        <div class="card bg-white shadow-md rounded-lg px-4 py-4 mb-6 ">
                            <form action="{{ route('places.update',$place->id)}}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="flex items-center justify-center">
                                  ...
                                  <input  id="title" name="title" type="text" value="{{ $place->title }}" value="{{ old('title')}}" class="rounded px-4 w-full py-1 bg-gray-200  border border-gray-400 mb-3 text-gray-700 placeholder-gray-700 focus:bg-white focus:outline-none"/>
                                     @error('title')
                                  ...
                                  <select id="plc_type" name="plc_type" value="{{ $place->plc_type }}" class="rounded px-4 w-full py-1 bg-gray-200  border border-gray-400 mb-3 text-gray-700 placeholder-gray-700 focus:bg-white focus:outline-none">
                                      ...
                                      <option value="private office" {{ ($place->plc_type=="private office")? "selected" : "" }} {{ old('plc_type') == "private office" ? 'selected' : '' }} >Private Office</option>
                                      <option value="meeting room" {{ ($place->plc_type=="meeting room")? "selected" : "" }} {{ old('plc_type') == "meeting room" ? 'selected' : '' }} >Meeting Room</option>
                                  </select>
                                  ...
                                  <input id="price" name="price" type="number" value="{{ $place->price }}" value="{{ old('price')}}" class="rounded px-4 w-full py-1 bg-gray-200  border border-gray-400 mb-3 text-gray-700 placeholder-gray-700 focus:bg-white focus:outline-none"/>
                                       @error('price')
                                         <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                                       @enderror

                                  <label for="description"  class="inline-block text-gray-500">Description:</label>
                                  <textarea id="description" name="description"  type="textarea" class="rounded px-4 w-full py-1 bg-gray-200  border border-gray-400 mb-3 text-gray-700 placeholder-gray-700 focus:bg-white focus:outline-none">{{ $place->description }} {{old('description')}}</textarea>
                                   @error('description')
                                     <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                                   @enderror
                                                ...
                                                <label
                                                    ...
                                                    <div class="flex flex-col items-left w-full justify-center pt-0">
                                                       ...
                                                       <div class="flex ">
                                                        ...
                                                        <img src="{{\Storage::url($place->image)}}" width="150" height="100"  alt="old image">
                                                    </div>
                                                    </div>
                                                    @error('image')
                                                    <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                              ...
                              <div class=" px-2 py-3 bg-white text-right sm:px-6 flex justify-between">
                                    <a href="{{ route('dashboard.places_index')}}" class="inline-flex w-20 justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-indigo-700 bg-indigo-100 hover:bg-indigo-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                        Cancel
                                      </a>
