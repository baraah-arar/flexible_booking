...
<x-modal-overlay id="calendar-overlay">
    <x-modal-header closeDes=''>{{__('Click to set start & end time for your reservation')}}</x-modal-header>
                ...
                if(check_in_input.value != '' && check_out_input.value != '')
                    ...
                    if(check_in_input.value == ''){
                        ...
                        if(new Date(cell.dataset.dateTime) < curr_date)
                            document.querySelector('.error_message').innerText = `{{__('Not valid date')}}`;
                        else{
