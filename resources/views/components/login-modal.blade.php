<x-modal-overlay id="login-modal">
    <x-modal-header>{{__('Welcome back')}}</x-modal-header>
    <div class="modal-body">
        <p class="text-sm text-gray-500">{{__('welcome one more time, please Enter your Information.')}}</p>
         <form class="mt-8 space-y-6" action="/login" method="POST">
            ...
            <x-form.submit-btn>{{__('login')}}</x-form.submit-btn>
        </form>
    </div>
    <div class="modal-footer flex justify-center items-center space-x-2 border-t border-gray-300 py-4">
        <p class="text-sm text-gray-500">{{__("Or you don't have account")}}</p>
        <a href='#register' class="font-medium text-indigo-600 hover:text-indigo-500">{{__('Register Now')}}</a>
    </div>
</x-modal-overlay>